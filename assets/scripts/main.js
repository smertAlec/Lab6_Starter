...
function getRecipesFromStorage() {
	...
	return localStorage.getItem('recipes') !== null ? JSON.parse(localStorage.recipes) : [];
}
...
function initFormHandler() {
	...
	//            values from the FormData object and insert them into recipeObject
		...
		for(const pair of form_data.entries()) {
			recipe_object[pair[0]] = pair[1];
		}

	// B6. TODO - Create a new <recipe-card> element
		let recipe = document.createElement('recipe-card');
	// B7. TODO - Add the recipeObject data to <recipe-card> using element.data
		recipe.data = recipe_object;
	// B8. TODO - Append this new <recipe-card> to <main>
		let main_el = document.querySelector('main');
		main_el.append(recipe);
	// B9. TODO - Get the recipes array from localStorage, add this new recipe to it, and
	//            then save the recipes array back to localStorage
		let recipe_list = getRecipesFromStorage();
		recipe_list.push(recipe_object);
